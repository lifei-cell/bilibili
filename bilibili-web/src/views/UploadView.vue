...
async function publish() {
  ...
  error.value = ''
  try {
    const result = await videoApi.publish({ title: form.value.title, description: form.value.description, coverUrl: form.value.coverUrl || undefined, sourceUrl: sourceUrl.value, fileMd5: fileMd5.value, fileSize: file.value.size, duration: duration.value, resolution: resolution.value, categoryId: form.value.categoryId, tags })
    phase.value = 'done'
    setTimeout(() => router.push(`/video/${result.data.videoId}`), 900)
