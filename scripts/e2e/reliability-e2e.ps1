...
try {
    ...
    $check = Invoke-BiliApi -Method POST -Uri 'http://localhost:8080/api/upload/check' -Headers $authHeaders -Body @{
        fileMd5 = $fileMd5; fileName = 'reliability-e2e.mp4'; fileSize = $fileSize; totalChunks = 1
    }
    $uploadId = $check.data.uploadId
    Assert-True (![string]::IsNullOrWhiteSpace($uploadId)) 'Upload check did not create a task'

    $uploadResponse = & curl.exe -sS -X POST 'http://localhost:8080/api/upload/chunk' `
        -H "satoken: $token" -F "uploadId=$uploadId" -F "fileMd5=$fileMd5" `
        -F "chunkMd5=$fileMd5" -F 'chunkIndex=0' -F "chunkSize=$fileSize" `
        -F 'totalChunks=1' -F "file=@$videoPath;type=video/mp4"
    if ($LASTEXITCODE -ne 0) { throw 'Chunk upload failed' }
    $uploaded = $uploadResponse | ConvertFrom-Json
    Assert-True $uploaded.success "Chunk upload business failure: $($uploaded.errorMsg)"

    $merge = Invoke-BiliApi -Method POST -Uri 'http://localhost:8080/api/upload/merge' -Headers $authHeaders -Body @{
        uploadId = $uploadId; fileMd5 = $fileMd5; fileName = 'reliability-e2e.mp4'; totalChunks = 1
    }
    ...
    Invoke-BiliApi -Method POST -Uri "http://localhost:8080/api/admin/video/$videoId/publish" -Headers $adminHeaders | Out-Null
    ...
    if ($started -and $videoId -gt 0) {
        try {
            $cleanupSql = "delete from mq_consumed_message where topic='video-view' and message_key like '${videoId}:%'; delete from mq_consumed_message where topic='danmu-persist' and message_key='$danmuId'; delete i from mq_consumed_message i join reliable_event_outbox o on i.topic=o.topic and i.message_key=o.event_id where o.aggregate_id='$videoId'; delete from reliable_event_outbox where aggregate_id='$videoId' or (aggregate_type='danmu' and aggregate_id='$danmuId'); delete from danmu where video_id=$videoId; delete from comment where video_id=$videoId; delete from user_like where target_type=1 and target_id=$videoId; delete from collection where video_id=$videoId; delete from video_stats where video_id=$videoId; delete from video where id=$videoId; delete from video_transcode_task where file_md5='$fileMd5'; delete from file_chunk where file_md5='$fileMd5';"
            & docker exec bilibili-mysql mysql -uroot -proot bilibili -e $cleanupSql 2>$null | Out-Null
            & docker run --rm --network bilibili-net --entrypoint sh minio/mc -c "mc alias set local http://minio:9000 minioadmin minioadmin >/dev/null; mc rm --force local/videos/source/$fileMd5.mp4 local/videos/play/$fileMd5.mp4 local/tmp/$uploadId/0 >/dev/null 2>&1 || true" | Out-Null
